...
internal static class ExplainCommand
{
    ...
    private static int ExplainLockFiles()
    {
        ...
        AnsiConsole.MarkupLine("[grey]       </PropertyGroup>[/]");
        AnsiConsole.MarkupLine("[grey]  2. Run: [bold white]dotnet restore --force-evaluate --configuration Release[/][/]");
        AnsiConsole.MarkupLine("[grey]     Generates packages.lock.json. Use --configuration Release (or whichever[/]");
        AnsiConsole.MarkupLine("[grey]     config CI uses) -- the SDK injects different packages in Debug vs. Release[/]");
        AnsiConsole.MarkupLine("[grey]     for some project types (e.g. Blazor WASM hot-reload package). Committing[/]");
        AnsiConsole.MarkupLine("[grey]     a Debug lock file and restoring in Release (or vice versa) causes NU1004.[/]");
        AnsiConsole.MarkupLine("[grey]  3. Add global.json to the repo root to pin the SDK version:[/]");
        AnsiConsole.MarkupLine("[grey]       { \"sdk\": { \"version\": \"10.0.xxx\", \"rollForward\": \"disable\" } }[/]");
        AnsiConsole.MarkupLine("[grey]     Prevents SDK patch drift -- implicit packages like[/]");
        AnsiConsole.MarkupLine("[grey]     Microsoft.NET.ILLink.Tasks are versioned to the SDK patch. Two developers[/]");
        AnsiConsole.MarkupLine("[grey]     (or a developer and CI) on different SDK patches produce different lock[/]");
        AnsiConsole.MarkupLine("[grey]     files, causing NU1004 failures. nuget-audit does not check for this.[/]");
        AnsiConsole.MarkupLine("[grey]  4. Commit:[/]");
        AnsiConsole.MarkupLine("[grey]       git add packages.lock.json Directory.Build.props global.json[/]");
        Console.WriteLine();
